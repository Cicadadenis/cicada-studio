...
// Подстановка токена из профиля при вставке кода
function injectTokenIntoCode(code, currentUser) {
  const token = (currentUser?.test_token || '').trim();
  if (!token) return code;
  // Заменяем различные плейсхолдеры токена на реальный токен
  return code
    .replace(/бот\s+"YOUR_BOT_TOKEN"/g, `бот "${token}"`)
    .replace(/бот\s+"0000000000:PASTE_YOUR_BOTFATHER_TOKEN_HERE"/g, `бот "${token}"`);
}
...
function ModuleLibraryModal({ onClose, onInsert, onUpgrade, currentUser, t = getConstructorStrings('ru'), lang = 'ru' }) {
  ...
  const handleInsert = useCallback(() => {
    ...
    if (!item) return;
    const codeWithToken = injectTokenIntoCode(item.code, currentUser);
    onInsert(codeWithToken);
    onClose();
  }, [selected, selectedItem, onInsert, onClose, currentUser]);
